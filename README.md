...
`status` summarizes protocol health, asset execution state, forge task progress, runtime availability, and recommended next actions. Use `--json` when an agent, UI, or automation needs the same state as structured data.
