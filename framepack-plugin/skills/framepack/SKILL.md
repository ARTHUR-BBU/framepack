...
# Framepack v0.17.0 — HyperFrames 0.7.21 Director Workbench
...
## v0.17.0: Director Workbench for HyperFrames 0.7.21
...
- `core/quality_audit.py` — 纯 Python 语义审计，检查 stale `.framepack/arsenal.json`、Execution Manifest 武器缺登记、HANDWRITE reason 是否被 MOC 明显武器反证（覆盖 21 件非 deprecated builtin 可执行武器；`transitions-pack` deprecated 不强制）、手动 `data-hf-id`、Manifest 参数与 HTML 函数实参漂移、card-cascade 未声明、外部 Google Fonts 运行时依赖、缺失本地字体资产、暗底低可见性风险等问题。
