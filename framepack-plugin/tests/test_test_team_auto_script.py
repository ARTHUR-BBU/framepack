"""Test-team v0.18.0 auto-test script tests."""
...
def test_test_team_auto_script_dry_run_emits_plan(tmp_path):
    ...
    assert payload["framepack_version"] == "0.18.0"
...
def test_test_team_auto_script_writes_json_report_for_dry_run(tmp_path):
    ...
    report = json.loads((output_dir / "framepack-v0160-auto-test-report.json").read_text(encoding="utf-8"))
    assert report["framepack_version"] == "0.18.0"
