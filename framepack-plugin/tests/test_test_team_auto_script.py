"""Test-team v0.10.6 auto-test script tests."""
...
SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "test_team_v0106_auto_test.py"


def _load_script_module():
    spec = importlib.util.spec_from_file_location("test_team_v0106_auto_test", SCRIPT)
...
def test_test_team_auto_script_dry_run_emits_plan(tmp_path):
    ...
    assert payload["framepack_version"] == "0.10.6"
...
def test_test_team_auto_script_writes_json_report_for_dry_run(tmp_path):
    ...
    report = json.loads((output_dir / "framepack-v0106-auto-test-report.json").read_text(encoding="utf-8"))
    assert report["framepack_version"] == "0.10.6"
