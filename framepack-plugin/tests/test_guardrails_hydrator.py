...
def make_plugin(tmp_path: Path, version: str = "0.9.3", guardrails: str = "# Framepack Guardrails\n\n最新铁律") -> Path:
...
def test_sync_repairs_body_drift_even_if_header_hash_matches(tmp_path):
    ...
    project_dir.mkdir()
    tampered = (
        f"<!-- FRAMEPACK MANAGED BLOCK START version=0.9.3 hash={payload.digest} source=plugin -->\n"
        "# Framepack Guardrails\n\n被手改坏的规则\n"
        "<!-- FRAMEPACK MANAGED BLOCK END -->\n"
    )
...
def test_sync_repairs_stale_version_even_if_hash_matches(tmp_path):
    ...
    plugin_dir = make_plugin(tmp_path, version="0.9.3", guardrails="# Framepack Guardrails\n\n同一份规则")
    payload = build_guardrails_payload(plugin_dir)
    project_dir = tmp_path / "project"
    project_dir.mkdir()
    stale_version_block = payload.block.replace("version=0.9.3", "version=0.9.2")
