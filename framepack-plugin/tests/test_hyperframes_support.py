...
def test_supported_version_allows_normal_handoff():
    decision = classify_hyperframes_version("0.6.97", window())
...
def test_too_old_version_recommends_upgrade_and_blocks_handoff():
    decision = classify_hyperframes_version("0.6.85", window())
...
def test_hard_too_old_version_blocks_handoff():
    decision = classify_hyperframes_version("0.6.70", window())
...
def test_newer_patch_in_same_soft_band_requires_smoke_before_guarded_handoff():
    before_smoke = classify_hyperframes_version("0.6.108", window())
    ...
    after_smoke = classify_hyperframes_version("0.6.108", window(), smoke_passed=True)
...
def test_unknown_newer_minor_or_major_uses_discovery_only_until_smoke_passes():
    before_smoke = classify_hyperframes_version("0.7.0", window())
    ...
    failed_smoke = classify_hyperframes_version("0.7.0", window(), smoke_passed=False)
