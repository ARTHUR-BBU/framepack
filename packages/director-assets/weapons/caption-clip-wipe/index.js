export function captionClipWipe(tl, textEl, opts = {}, position = '>') {
  ...
  words.forEach((word, index) => tl.fromTo(word, { clipPath: clips[direction], opacity: 0 }, { clipPath: 'inset(0 0 0 0)', opacity: 1, duration: durationPerWord, ease: 'power2.out' }, `${position}${index ? `+=${staggerPerWord}` : ''}`));
  return tl;
}
