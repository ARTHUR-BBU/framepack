export function textSplitEnter(tl, textEl, opts = {}, position = '>') {
  ...
  if (splitMode === 'char') {
    ...
    tl.fromTo(chars, { autoAlpha: 0, y: sign * -60, rotationX: -40 }, { autoAlpha: 1, y: 0, rotationX: 0, duration, ease: 'back.out(1.2)', stagger: staggerPerChar }, position);
    return tl;
  }
  ...
  tl.fromTo(first, { [axis]: sign * -travelDistance, autoAlpha: 0 }, { [axis]: 0, autoAlpha: 1, duration, ease: 'power3.out' }, position);
  tl.fromTo(second, { [axis]: sign * travelDistance, autoAlpha: 0 }, { [axis]: 0, autoAlpha: 1, duration, ease: 'power3.out' }, position);
  return tl;
}
